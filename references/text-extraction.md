...
- `extraction/raw_pages.json`: line-level page text with bounding boxes.
...
Before publishing, compare the final question against `raw_source_text`/page render and set `extraction.text_reviewed: true`. Any uncertain symbol, missing line, ambiguous choice label, scan-only page, or suspected truncation remains `requires_manual_review: true` until resolved.
