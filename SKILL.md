...
Create a `question-bank/` bundle whose `questions.json` records each question's ID, source PDF/page/original number, `content_format`, stem, choices, answer status, knowledge points, and asset IDs. Use `content_format: "markdown+latex"`: prose is Markdown and all mathematical notation uses `$...$` inline or `$$...$$` as display math. JSON stores the LaTeX source; a later renderer must use MathJax, KaTeX, or an equivalent math-capable renderer to display it.

Keep the `answer` data field even when no answer is supplied, for example `{"text": null, "label": null, "evidence": "not-provided"}`. Only record answers from a supplied official key or user-provided source. If the source lacks answers, do **not** derive, infer, solve, or invent them unless the user explicitly requests solutions. Keep source text and final rendered assets under `assets/`; a question may reference a stem diagram, option diagram(s), or a shared asset. Preserve source wording unless the user authorizes adaptation.
...
1. **Inventory the source.** Record source files and supplied answer keys. Extract selectable text where reliable; otherwise mark the question for transcription/OCR review. Identify question boundaries before claiming figure ownership.
2. **Create the question records first.** Establish stable IDs, page/original question number, `content_format: "markdown+latex"`, answer status, topic tags, and `asset_ids`. If no answer key exists, set the answer evidence to `not-provided` and continue; do not spend time solving missing answers. Do not embed a whole PDF-page screenshot when a stem and choices can be typeset.
3. **Discover candidate graphics independently of captions.** Research papers often have `Figure N`; exam PDFs commonly do not. Run:
   ...
   ```bash
   python "$SKILL_DIR/scripts/discover_pdf_assets.py" source.pdf --outdir "$TMPDIR/question-bank-discovery" --zoom 6
   ```
...
5. **Crop safely.** Use the included crop utility against a candidate PNG. It preserves the original as `_debug/<name>.bak` before destructive edits and retains marked/preview iterations. Perform this work in a temporary processing directory, then copy only the reviewed final crop into `assets/figures/`.
...
7. **Validate the bundle.** Every `asset_id` in `questions.json` must resolve to an existing file and every asset must have one or more owners (or `shared_with`). Validate that mathematical expressions use the declared Markdown+LaTeX convention. Verify answers only against an official or user-supplied key; missing answers remain `not-provided`. Keep student-facing exports free of internal source notes unless the user asks to show them.
...
- `assets/source/` and final `assets/figures/` belong in the delivered bundle. Discovery renders, candidate images, crop backups, and `_debug/` previews are temporary audit material: keep them outside the bundle or remove them after final visual QA unless the user explicitly requests an audit package.
